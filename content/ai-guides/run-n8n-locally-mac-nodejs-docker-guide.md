...
Running n8n locally on your Mac gives you unlimited workflow executions, full data control, and the ability to test automations before deploying anywhere. n8n Cloud starts at €24 per month for 2,500 executions — running it locally costs nothing beyond your existing hardware. This guide covers two installation paths (Node.js and Docker), migrating your existing Cloud workflows, connecting n8n to Claude, and the most common errors you will encounter.
...
> **Which option should you choose?** Node.js is faster to set up and ideal for trying n8n for the first time. Docker is the better long-term choice: your data persists reliably, updates are simpler, and it matches how n8n is deployed in production.
...
Both commands should return a version number. Node.js **v20.19 or higher** is required for n8n to run correctly.
...
This installs n8n as a global command on your system. The download is around 300–400 MB and may take a few minutes depending on your connection.
...
n8n will start and print output in your Terminal. Open your browser and go to:
...
The first time you open n8n locally, it prompts you to create an owner account with an email and password. This is stored locally — no data is sent to n8n servers.
...
Download Docker Desktop for Mac from [docker.com/products/docker-desktop](https://www.docker.com/products/docker-desktop) and install it. Open Docker Desktop and wait until the status indicator shows it is running before continuing.
...
The `-v` flag is the most important one. Without it, all your data disappears the moment the container stops.
...
Repeat for each workflow. Credentials are not included in exports — you will need to re-enter API keys and connection details in your local instance after importing.
...
Add an **AI Agent** node or **Anthropic Chat Model** node to your canvas and select the credential you just created. Claude can then summarise content, classify data, draft text, or take action based on what upstream nodes send it.
...
All n8n data — workflows, credentials, execution history, and the encryption key used to protect your credentials — lives in `~/.n8n` on your Mac.
...
You now have a fully local n8n instance running on your Mac, with data persisting between sessions and ready to connect to your existing tools and API credentials.

Three things to try next:

- **Automate content workflows:** pull from an RSS feed, summarise each article with Claude, and post the output to WordPress or Notion

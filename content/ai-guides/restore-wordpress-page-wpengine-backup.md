...
WordPress has no native "restore single page from backup" feature. WP Engine's backup system saves your entire site, database and files, as one package. To restore one page, you must go into the raw database dump and extract it yourself.

This post documents exactly how to do this: extract a single page from a WP Engine full-site database backup and restore it to WordPress via the REST API, section by section, without touching anything else on the site.

## The problem

The page in question was a proposal writing service page, a custom-built page with 11 distinct content blocks including a hero section, lifecycle diagram, two testimonial carousels, a testimonials grid, and a contact form. During an earlier restoration attempt, only the hero block was pushed. Everything else was missing.

> **Core challenge:** WordPress does not let you restore a single page from a backup. WP Engine saves your entire site as one package. To restore one page, you must go into the raw database dump and extract it yourself.
...
> If you use Claude with the WordPress MCP tool, you already have REST API write access built in. This workflow uses that access to push the restored content.

## Step 1: Locate the database backup file

In WP Engine, go to **Sites → Backups → Download**. Unzip the archive. The file you need is `mysql.sql`, found at `wp-content/mysql.sql`. At 80MB or more, work with it via a script rather than open it in a text editor.
...
> Always read back the **`raw` field** from the API response, not `rendered`. The rendered version applies WordPress filters and looks different from your source. Raw shows exactly what was saved.

## Step 5: Handle large pages in accumulating sections

If your page is large, for example **24,323 characters across 11 blocks**, push in sections. Push blocks 1-5, confirm the raw content saved correctly, then push 1-5 plus block 6, and so on. Each call replaces the entire page content, so always send the full accumulated content, not just the new section.
...
This method restores the `post_content` field only. The following items sit outside that field and need separate handling:

- **Custom page CSS** stored in post meta (for example, `_uag_custom_page_level_css`) requires a separate meta update call
- **Yoast SEO meta fields**, such as title, description, and schema settings, live separately
- **Featured image assignments** and **page template settings** sit in post meta, not post content

> If your *entire site* needs restoring, use WP Engine's one-click restore instead. This manual approach is specifically for restoring a single page's content without touching anything else. It causes no downtime and no risk to other pages.
...
This guide accompanies the [full WP Engine page restore post on The Science Talk](https://thesciencetalk.com/ai-academy/restore-wordpress-page-wpengine-backup/), covering how this situation arose, why a full restore was not an option, and what the WordPress MCP made possible.
